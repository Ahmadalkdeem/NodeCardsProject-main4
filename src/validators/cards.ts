...
const schema = joi.object({
  ...
  fSizeOptions2: joi.array().items(joi.object({ size: joi.string().regex(size).required(), colors: joi.array().items(joi.object({ color: joi.string().regex(color).required() })).required().min(1).max(25) }).required()).min(1).max(25)
});
